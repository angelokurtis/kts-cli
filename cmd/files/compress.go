...
func compress(_ *cobra.Command, args []string) {
	...
	for _, f := range fileList {
		err = runPBZIP2Compress(ctx, dir, f.path)
		check(err)
...
func runPBZIP2Compress(ctx context.Context, workingDir, path string) error {
	// Define the shell script as a string
	shellScript := fmt.Sprintf(`
	#!/bin/bash
	set -e
	tar -caf "%s.tar.bz2" --use-compress-prog=pbzip2 --checkpoint=100 --checkpoint-action=dot "%s"
	`, path, path)

	color.Primary.Println(fmt.Sprintf(`tar -caf "%s.tar.bz2" --use-compress-prog=pbzip2 --checkpoint=100 --checkpoint-action=dot "%s"`, path, path))
