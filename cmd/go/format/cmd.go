...
func runFormat(cmd *cobra.Command, args []string) error {
	...
	for _, file := range files {
		...
		slog.DebugContext(ctx, "Copied file", slog.String("src", src), slog.String("dst", dst))
	}
	...
	slog.DebugContext(ctx, "Running goimports-reviser", slog.String("runArgs", strings.Join(runArgs, " ")))
	...
	slog.DebugContext(ctx, "Running gofumpt", slog.String("runArgs", strings.Join(runArgs, " ")))
	...
	slog.DebugContext(ctx, "Running wsl", slog.String("runArgs", strings.Join(runArgs, " ")))
	...
	for _, file := range files {
		...
		slog.DebugContext(ctx, "Copied file back", slog.String("src", src), slog.String("dst", dst))
	}
