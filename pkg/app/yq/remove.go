...
func UpdateNode(yamlPath, pathExpression, value string) error {
	_, err := bash.Run("yq write -i " + yamlPath + " " + pathExpression + " " + value)
	return err
}

func UpdateNodeWithQuotes(yamlPath, pathExpression, value string) error {
	_, err := bash.Run("yq write -i " + yamlPath + " " + pathExpression + " --style=double " + value)
	return err
}

func ReadNodeValues(yamlPath, pathExpression string) ([]string, error) {
